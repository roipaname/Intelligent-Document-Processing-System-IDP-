...
class TesseractOCR:
    """
     ...
     -Bouding Box Extraction

     """
    
    def __init__(self,tesseract_cmd:Optional[str]=None,language:str='eng',dpi:int=500,psm:int=6):
        """
        ...
        """
        if tesseract_cmd:
            pytesseract.pytessearct.tesseract_cmd=tesseract_cmd
        self.language=language
        self.dpi=dpi
        self.psm=psm
        
        self.config=f"--oem 3 --psm {self.psm}"
        logger.info(f"Initialized Tesseract OCR Engine : lang={self.language}, dpi={self.dpi}, psm={self.psm}")
    ...
    def _process_image(self,image:Image.Image,page:int)->OCRResult:
        ...
        #Extract detailed data with boudning boxes
    ...
    def extract_from_pdf(self,pdf_path:str)->List[OCRResult]:
        """
        Docstring for extract_from_pdf
    ...
    def extract_from_image(self,image_path:Path)->List[OCRResult]:
